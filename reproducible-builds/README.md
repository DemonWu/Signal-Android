...
```bash
adb pull $(adb shell pm path com.ytqf.signal | grep /base.apk | awk -F':' '{print $2}') ~/reproducible-signal/apk-from-google-play-store/Signal-$(adb shell dumpsys package com.ytqf.signal | grep versionName | awk -F'=' '{print $2}').apk
```
...
```bash
adb shell pm path com.ytqf.signal
```

This will output something like:

```bash
package:/data/app/com.ytqf.signal-aWRzcGlzcG9wZA==/base.apk
```

The output will tell you where the Signal APK is located in your device. (In this example the path is `/data/app/com.ytqf.signal-aWRzcGlzcG9wZA==/base.apk`)
...
```bash
adb pull \
  /data/app/com.ytqf.signal-aWRzcGlzcG9wZA==/base.apk \
